...
class ShopController extends Controller
{
            ...
            switch ($this->create($request->all(), true)) {
                case 0:
                    return redirect()->back()
                        ->with('success', 'The shop has been added.');
                    break;

                case 1:
                    return redirect()->back()
                        ->withErrors([
                            'noid' => 'The franchise you entered does not exist in our database.',
                        ]);
                    break;
            }
    ...
    public function deleteShop(Request $request)
    {
        if (Auth::user()->login_type == 3 || Auth::user()->login_type ==4) {
            ...
            return redirect()->back()
                ->with('success', 'The shop has been deleted.');
        } else {
            return redirect()->to("unauthorized");
        }
    }
    ...
    private function validator(array $data, $id)
    {
        ...
        if ($id) {
            ...
            $messages = array(
                ...
                'name.alpha' => 'Plese enter a valid Name',
            ...
            $messages = array(
                'franchise_id.required' => 'Shop is required.',
                'franchise_id.alpha' => 'Please enter a valid Shop name.',
                'franchise_id.max' => 'Please enter a shorter Franchise Name.',
                'name.required' => 'Name is required.',
                'name.alpha' => 'Plese enter a valid Name',
                'name.max' => 'Name must be smaller than 255 characters.',
                'location.required' => 'Address is required.'
