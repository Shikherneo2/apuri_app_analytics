...
class Shop_employeeController extends Controller {
	...
	public function addEmployeeFromCSV(){
		if( Auth::user()->login_type <= 3 || Auth::user()->login_type == 5){
	        ...
	        if( is_null($uploads) ){
	            return redirect()->back()
						->withErrors([
							'nocsv' => 'おっとっと!　どちらでもＣＳＶファイルはありません。',
						]);
	        }
	        else{
	        	$csv_problem = false;
	        	...
	        	if($file!=false){
					...
					if($csv_problem===true){
						...
						foreach($data as $user){
							...
							if ($validator->fails()){
								return redirect()->back()
												 ->withErrors([
												 	 'error' => 'おっとっと!　there is problem with the input'.$validator->messages()->toJson(),
												 	]);
							}
							...
							switch( $this->create($user, true) ){
								case 1: return redirect()->back()
														 ->withErrors([
														 	 'error' => 'おっとっと!　there is no such shop　－　"'.$user["shop_id"].'"',
														 	]);
							}
						}
					...
					return redirect()->back()->withErrors(['error' => 'おっとっと!　file could not be opened']);
				}
	    	}
	    }
	    else{
	...
	private function validator(array $data, $id){
		...
		if($id){
			...
			$messages = array(
	    		...
	    		'name.required' => '名前が必要です。',
	    		'name.max' => '名前の長さは255文字以下のなければなりません。。',
	    		'shop_id.required' => 'ショップが必要です。',
	    		'shop_id.string' => '入力したショップの名前は無効です。',
	    		'shop_id.max' => 'ショップの名前の長さは255文字以下のなければなりません。'
			);
			return Validator::make($data, [
				'name' => 'required|max:255',
				'email' => 'required|email|max:255|unique:users',
    ...
    /**
	 * View Shop Employee Records - JSON for DataTables
	 *
	 * @param  void
	 * @return JSON
	 */
