...
class AppController extends Controller {
		...
		if( Auth::user()->login_type <= 3 ){
			...
			switch( $this->create($request->all(), true) ){
				case 1: return redirect()->back()
							 ->withErrors([
								'exists' => '他のアプリも　同じパッケージの名前とプラットフォームが持っています。もう一回　してみてください。',
							]);
							break;	
				case 0: return redirect()->back()
					   		->with('success','アプリが加えました。');
			}
		}
		else{
		...
		if( Auth::user()->login_type <= 3 ){
			...
			if( !is_null($app) ){
				...
				return redirect()->back()
						   		->with('success','アプリが更新しました。');
			}
			else{
				return redirect()->back()
								 ->withErrors([
									'exists' => 'あのアプリは私たちのデータベースでじゃない。',
								]);
			}
		}
		else
	...
	private function validator(array $data, $id){
		$messages = array(
    		'package_name.required'  => 'パッケージが必要です。',
    		'package_name.max'  => 'パッケージの長さは 7桁のなければなりません。 ',
    		'type.required' => 'プラットフォームが必要です。',
    		'name.required' => 'アプリの名前が必要です。',
    		'name.max' => 'アプリの名前の長さは40文字以下のなければなりません。',
    		'store_link.required' => 'ストアのリンクが必要です。',
    		'store_link.max' => 'ストアのリンクの長さは2083文字以下のなければなりません。'
		); 
